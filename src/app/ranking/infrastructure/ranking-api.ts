...
interface UserRow { id: number; name: string; ecopoints: number; [k: string]: any; }
interface CosmeticRow { id: number; type: string; image_url: string; [k: string]: any; }
interface UserCosmeticRow { user_id: number; cosmetic_id: number; equipped: boolean; [k: string]: any; }
interface TxRow { id: number; user_id: number; ecopoints: number; created_at: string; [k: string]: any; }
...
export class RankingApiService {
  ...
  getAllRankingEntries(): Observable<{ now: Date; data: Map<number, RankingEntry[]> }> {
    return this.timeApi.getCurrentTime().pipe(
      switchMap(now => {
        ...
        [1, 2, 3].forEach(id => { ranges[id] = this.timeApi.calculateDateRange(now, id); });
  ...
  private reconcileTransactions(now: Date, ctx: RankingData): Observable<RankingData> {
    ...
    ctx.users.forEach((user: UserRow) => {
      ...
      if (diff > 0) {
        ops.push(
          this.http.post<TxRow>(`${this.baseUrl}/ecopoint_transaction`, {
            user_id: user.id,
            ecopoints: diff,
            created_at: now.toISOString(),
          }).pipe(
            map((created: TxRow) => { reconciled.push(created); return created; })
          )
        );
      }
    });
    ...
    return forkJoin(ops).pipe(
      map(() => ({ ...ctx, transactions: reconciled }))
    );
  }
  ...
  private buildRankings(
    ...
    const userData = new Map<number, {
      ...
      overlayUrl: string | null; overlayType: string | null;
    }>();
